...
__global__ void gemm_v1(const float* a,
                        const float* b,
                        float* c,
                        const uint32_t M,
                        const uint32_t N,
...
int main(int argc, char **argv) {
  ...
  int blocks = 1;
  int threads = 256;
  cudaOccupancyMaxPotentialBlockSize(&blocks, &threads, gemm_v1, 0, 0);
  ...
  gemm_v1<<<blocks, threads, 0, 0>>>(dev_a_ptr, dev_b_ptr, dev_c_ptr, M, N, K);
