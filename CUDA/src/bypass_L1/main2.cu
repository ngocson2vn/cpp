...
__device__ bool has_done(bool* ptr) {
  int status = 0;
  asm volatile(
...
__global__ void add_vectors(const float *__restrict__ a,
                            ...
                            bool *__restrict__ stats,
                            int blockSize, int totalElems) {
  auto blockIndex = blockIdx.x;

  while (blockIndex > 0 && !has_done(&stats[blockIndex - 1])) {
    __nanosleep(1000);
  }
...
int main(int argc, char **argv) {
  ...
  bool *dev_stats = nullptr;
  CHECK_CUDA_ERROR(cudaMalloc(&dev_stats, 2));
  ...
  CHECK_CUDA_ERROR(
      cudaMemcpy(dev_stats, init_stats, 2, cudaMemcpyHostToDevice));
