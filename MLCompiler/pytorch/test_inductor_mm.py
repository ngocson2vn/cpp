...
debug_dir = os.getenv("TORCH_COMPILE_DEBUG_DIR", None)
if not debug_dir:
  print("TORCH_COMPILE_DEBUG_DIR is not set")
  sys.exit(1)
...
if __name__ == "__main__":
  ...
  y1 = (torch.rand(256, 64).cuda() - 0.5)
  res1 = toy(x1, y1)
  print(f"x1: {x1}")
  print(f"y1: {y1}")
  print(f"Result: {res1}\n")

  x2 = (torch.rand(16, 512).cuda() - 0.5)
  y2 = (torch.rand(512, 128).cuda() - 0.5)
  res2 = toy(x2, y2)
  print(f"x2: {x2}")
  print(f"y2: {y2}")
  print(f"Result: {res2}")
